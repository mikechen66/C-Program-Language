 ...
 * 
 * $gcc -o main *.c
 * main.c: In function ‘bfree’:
 * main.c:34:34: warning: cast from pointer to integer of different size [-Wpointer-to-int-cast]
 * #define ALIGN(p) (sizeof(Align)-((unsigned)(p)%sizeof(Align)))%sizeof(Align)
 *                                  ^
 * main.c:47:13: note: in expansion of macro ‘ALIGN’
 *     align = ALIGN(p);
 *             ^~~~~
 * main.c: In function ‘try_to_free’:
 * main.c:34:34: warning: cast from pointer to integer of different size [-Wpointer-to-int-cast]
 * #define ALIGN(p) (sizeof(Align)-((unsigned)(p)%sizeof(Align)))%sizeof(Align)
 *                                  ^
 * main.c:85:13: note: in expansion of macro ‘ALIGN’
 *     align = ALIGN(p->p);
 *             ^~~~~
