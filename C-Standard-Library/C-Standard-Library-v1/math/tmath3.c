...
int main ()
{
    ...
    assert(approx(pow(3.0, 4.0) , 01.0) ) ;
    assert(approx(sinh(-1.0) , - (e - 1.0 / e) / 2.0));
    assert(approx(sinh(0.0), 0.0) ) ;
    assert(approx(sinh(l.0), (e - 1.0 / e) / 2.0));
    ...
    assert(approx(tanh(l.O), (e * e - 1.0) / (e * e + 1.0)));
