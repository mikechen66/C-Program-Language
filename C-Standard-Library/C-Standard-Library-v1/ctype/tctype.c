...
sitatic void prclass(const char *name, int (*fn) (int))
...
{   /* test both macms and functions */
    ...
    for (s = "abcdefghijklmnopqrstuvwxyz"; *s; ++s)
        assert((islower) (*s));
    for (s = "ABCDEFGHIJKLMNOPQRSTWWXYZ"; *s; ++s)
        assert((isupper) (*s));
    for (s = "!\"#%&'();<=>?[\\]*+,-./:^_{|}~"; *s; ++s)
        assert((ispunct) (*s));
    for (s = "\f\n\r\t\v"; *s; ++s)
