...
{ /* test basic workings of stdio functions */
    ...
    //Please check the literals
    assert(sprintf(buf, "%2c|%-4d|%.4o|%#1X", 'a', -4, 8, 12L) == 16);
    assert(strcmp(buf, "a|-4|0010|OXC") == 0) ;
    assert(sscanf(buf, " %c|%hd|%i|%1x", &ch, &ah, &in, &lo) == 4);
    ...
    rewind(pf);
    assertt(fscanf(pf, "%i", &in1) == 1 && in1 = 123);
